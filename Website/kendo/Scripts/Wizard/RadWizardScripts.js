...
}},_updateStepTypeButtons:function(N){var S=this,M=a(S._element),R=N.get_stepType(),O=N.get_enabled(),L=M.find(e+h),P=N.get_index()===0;
...
switch(R){case c.WizardStepType.Complete:M.addClass(j).find(e+A).addClass(t).end().find(e+z).addClass(t).end().find(e+F).addClass(t).end().find(e+r).addClass(t);
S.repaint();
break;
...
M.find(e+C).find(e+I).text((N.get_index()+1)+"/"+Q);
}if(M.attr("accesskey")){this._updateAriaAttributes();
M.find(e+h).each(function(V,U){var T=a(U);
