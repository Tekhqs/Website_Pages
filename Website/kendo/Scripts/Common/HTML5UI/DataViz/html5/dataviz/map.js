...
function b(j){return d.format('<button class="k-button k-navigator-{0}"><span class="k-icon k-i-arrow-60-{0}"/></button>',j);
...
var g=f.keys;
var k=a.proxy;
function b(n,o){return f.format('<button class="k-button k-zoom-{0}" title="zoom-{0}"><span class="k-icon {1}"></span></button>',n,o);
...
this.createElement();
this.show();
},options:{urlTemplate:"",errorUrlTemplate:""},createElement:function(){this.element=a("<img style='position: absolute; display: block;' />").css({width:this.options.size,height:this.options.size}).on("error",n(function(g){if(this.errorUrl()){g.target.setAttribute("src",this.errorUrl());
...
}(function(){(function(a,A){var f=document,s=Math,t=s.min,w=s.pow,x=a.proxy,n=window.kendo,E=n.ui.Widget,d=n.deepExtend,c=n.dataviz,z=c.ui,m=n.geometry,v=m.Point,q=c.map,j=q.Extent,p=q.Location,i=q.crs.EPSG3857,B=n.util,y=B.renderPos,h=n.drawing.util,e=h.defined,o=h.limitValue,C=h.valueOrDefault;
var b="k-",k=0.9,l=0.9300000000000001,u="DOMMouseScroll mousewheel",D=5;
var r=E.extend({init:function(g,F){n.destroy(g);
E.fn.init.call(this,g);
this._initOptions(F);
this.bind(this.events,F);
this.crs=new i();
...
this._initLayers();
this._reset();
this._mousewheel=x(this._mousewheel,this);
this.element.bind("click",x(this._click,this));
this.element.bind(u,this._mousewheel);
...
},zoom:function(g){var F=this.options;
if(e(g)){g=s.round(o(g,F.minZoom,F.maxZoom));
if(F.zoom!==g){F.zoom=g;
this._reset();
}return this;
}else{return F.zoom;
}},center:function(g){if(g){this.options.center=p.create(g).toArray();
this._reset();
return this;
}else{return p.create(this.options.center);
}},extent:function(g){if(g){this._setExtent(g);
return this;
}else{return this._getExtent();
}},setOptions:function(g){E.fn.setOptions.call(this,g);
this._reset();
},locationToLayer:function(F,G){var g=!this.options.wraparound;
F=p.create(F);
return this.crs.toPoint(F,this._layerSize(G),g);
},layerToLocation:function(F,G){var g=!this.options.wraparound;
F=v.create(F);
return this.crs.toLocation(F,this._layerSize(G),g);
},locationToView:function(g){g=p.create(g);
var F=this.locationToLayer(this._viewOrigin);
var G=this.locationToLayer(g);
return G.translateWith(F.scale(-1));
},viewToLocation:function(F,G){var g=this.locationToLayer(this._getOrigin(),G);
F=v.create(F);
F=F.clone().translateWith(g);
return this.layerToLocation(F,G);
},eventOffset:function(g){var G=this.element.offset();
var F=g.originalEvent||g;
var H=C(F.pageX,F.clientX)-G.left;
var I=C(F.pageY,F.clientY)-G.top;
return new m.Point(H,I);
},eventToView:function(F){var g=this.eventOffset(F);
...
},eventToLocation:function(F){var g=this.eventOffset(F);
return this.viewToLocation(g);
},viewSize:function(){var g=this.element;
var F=this._layerSize();
var G=g.width();
if(!this.options.wraparound){G=t(F,G);
}return{width:G,height:t(F,g.height())};
},exportVisual:function(){this._reset();
return false;
},_setOrigin:function(g,H){var F=this.viewSize(),G;
g=this._origin=p.create(g);
G=this.locationToLayer(g,H);
G.x+=F.width/2;
G.y+=F.height/2;
this.options.center=this.layerToLocation(G,H).toArray();
return this;
},_getOrigin:function(g){var F=this.viewSize(),G;
if(g||!this._origin){G=this.locationToLayer(this.center());
G.x-=F.width/2;
G.y-=F.height/2;
this._origin=this.layerToLocation(G);
}return this._origin;
},_setExtent:function(g){g=j.create(g);
this.center(g.center());
var K=this.element.width();
var F=this.element.height();
for(var L=this.options.maxZoom;
L>=this.options.minZoom;
L--){var I=this.locationToLayer(g.nw,L);
var J=this.locationToLayer(g.se,L);
var H=s.abs(J.x-I.x);
var G=s.abs(J.y-I.y);
if(H<=K&&G<=F){break;
}}this.zoom(L);
},_getExtent:function(){var F=this._getOrigin();
var g=this.locationToLayer(F);
var H=this.viewSize();
g.x+=H.width;
g.y+=H.height;
var G=this.layerToLocation(g);
return new j(F,G);
},_zoomAround:function(F,g){this._setOrigin(this.layerToLocation(F,g),g);
this.zoom(g);
},_initControls:function(){var g=this.options.controls;
if(z.Attribution&&g.attribution){this._createAttribution(g.attribution);
}if(!n.support.mobileOS){if(z.Navigator&&g.navigator){this._createNavigator(g.navigator);
}if(z.ZoomControl&&g.zoom){this._createZoomControl(g.zoom);
}}},_createControlElement:function(F,g){var G=F.position||g;
var H="."+y(G).replace(" ",".");
var I=a(".k-map-controls"+H,this.element);
if(I.length===0){I=a("<div>").addClass("k-map-controls "+y(G)).appendTo(this.element);
}return a("<div>").appendTo(I);
},_createAttribution:function(F){var g=this._createControlElement(F,"bottomRight");
this.attribution=new z.Attribution(g,F);
},_createNavigator:function(G){var g=this._createControlElement(G,"topLeft");
var F=this.navigator=new z.Navigator(g,G);
this._navigatorPan=x(this._navigatorPan,this);
F.bind("pan",this._navigatorPan);
this._navigatorCenter=x(this._navigatorCenter,this);
F.bind("center",this._navigatorCenter);
},_navigatorPan:function(F){var H=this;
var I=H.scroller;
var K=I.scrollLeft+F.x;
var L=I.scrollTop-F.y;
var g=this._virtualSize;
var G=this.element.height();
var J=this.element.width();
K=o(K,g.x.min,g.x.max-J);
L=o(L,g.y.min,g.y.max-G);
H.scroller.one("scroll",function(M){H._scrollEnd(M);
});
H.scroller.scrollTo(-K,-L);
},_navigatorCenter:function(){this.center(this.options.center);
},_createZoomControl:function(F){var g=this._createControlElement(F,"topLeft");
var G=this.zoomControl=new z.ZoomControl(g,F);
this._zoomControlChange=x(this._zoomControlChange,this);
...
}},_initScroller:function(){var g=n.support.mobileOS?l:k;
var G=this.options.zoomable!==false;
var F=this.scroller=new n.mobile.ui.Scroller(this.element.children(0),{friction:g,velocityMultiplier:D,zoom:G,mousewheelScrolling:false});
F.bind("scroll",x(this._scroll,this));
F.bind("scrollEnd",x(this._scrollEnd,this));
F.userEvents.bind("gesturestart",x(this._scaleStart,this));
F.userEvents.bind("gestureend",x(this._scale,this));
this.scrollElement=F.scrollElement;
},_initLayers:function(){var F=this.options.layers,I=this.layers=[];
for(var G=0;
G<F.length;
G++){var J=F[G];
var K=J.type||"shape";
var g=this.options.layerDefaults[K];
var H=c.map.layers[K];
I.push(new H(this,d({},g,J)));
}},_initMarkers:function(){this.markers=new q.layers.MarkerLayer(this,this.options.markerDefaults);
this.markers.add(this.options.markers);
},_scroll:function(g){var H=this.locationToLayer(this._viewOrigin).round();
var F=g.sender.movable;
var G=new m.Point(F.x,F.y).scale(-1).scale(1/F.scale);
H.x+=G.x;
H.y+=G.y;
this._scrollOffset=G;
this._setOrigin(this.layerToLocation(H));
...
},_scaleStart:function(g){if(this.trigger("zoomStart",{originalEvent:g})){var F=g.touches[1];
if(F){F.cancel();
}}},_scale:function(G){var J=this.scroller.movable.scale;
var K=this._scaleToZoom(J);
var H=new m.Point(G.center.x,G.center.y);
var g=this.viewToLocation(H,K);
var F=this.locationToLayer(g,K);
var I=F.translate(-H.x,-H.y);
this._zoomAround(I,K);
this.trigger("zoomEnd",{originalEvent:G});
},_scaleToZoom:function(F){var g=this._layerSize()*F;
var G=g/this.options.minSize;
var H=s.log(G)/s.log(2);
return s.round(H);
...
this.trigger("reset");
},_resetScroller:function(){var G=this.scroller;
var J=G.dimensions.x;
var L=G.dimensions.y;
var F=this._layerSize();
var g=this.extent().nw;
var H=this.locationToLayer(g).round();
G.movable.round=true;
G.reset();
G.userEvents.cancel();
var N=this.zoom();
G.dimensions.forcedMinScale=w(2,this.options.minZoom-N);
G.dimensions.maxScale=w(2,this.options.maxZoom-N);
var K={min:-H.x,max:F-H.x};
var M={min:-H.y,max:F-H.y};
if(this.options.wraparound){K.max=20*F;
K.min=-K.max;
}if(this.options.pannable===false){var I=this.viewSize();
K.min=M.min=0;
K.max=I.width;
M.max=I.height;
}J.makeVirtual();
L.makeVirtual();
J.virtualSize(K.min,K.max);
L.virtualSize(M.min,M.max);
this._virtualSize={x:K,y:M};
},_renderLayers:function(){var F=this.options.layers,I=this.layers=[],K=this.scrollWrap;
K.empty();
for(var G=0;
G<F.length;
G++){var J=F[G];
...
var H=c.map.layers[L];
I.push(new H(this,d({},g,J)));
}},_layerSize:function(g){g=C(g,this.options.zoom);
return this.options.minSize*w(2,g);
},_click:function(F){if(!this._panComplete()){return;
}var g=this.eventOffset(F);
this.trigger("click",{originalEvent:F,location:this.viewToLocation(g)});
},_mousewheel:function(G){G.preventDefault();
var F=c.mwDelta(G)>0?-1:1;
var J=this.options;
var H=this.zoom();
var M=o(H+F,J.minZoom,J.maxZoom);
if(J.zoomable!==false&&M!==H){if(!this.trigger("zoomStart",{originalEvent:G})){var g=this.eventOffset(G);
var I=this.viewToLocation(g);
var L=this.locationToLayer(I,M);
var K=L.translate(-g.x,-g.y);
this._zoomAround(K,M);
this.trigger("zoomEnd",{originalEvent:G});
}}}});
c.ui.plugin(r);
}(window.kendo.jQuery));
