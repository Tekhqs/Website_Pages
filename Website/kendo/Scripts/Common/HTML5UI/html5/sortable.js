...
},_movementByAxis:function(w,x,z,A){var y=w==="x"?x.left:x.top,B=z<0?this.placeholder.prev():this.placeholder.next(),C;
if(B.length&&!B.is(":visible")){B=z<0?B.prev():B.next();
}b.extend(A,{target:B});
C=this._getElementCenter(B);
if(C){C=w==="x"?C.left:C.top;
}if(B.length&&z<0&&y-C<0){this._movePlaceholder({element:B,sortable:this},"prev",A);
}else{if(B.length&&z>0&&y-C>0){this._movePlaceholder({element:B,sortable:this},"next",A);
...
},_isFloating:function(w){return/left|right/.test(w.css("float"))||/inline|table-cell/.test(w.css("display"));
