...
},_resizeFilterInput:function(){var E=this.filterInput;
var G=this._prevent;
if(!E){return;
}var F=this.filterInput[0]===c();
var D=l.caret(this.filterInput[0])[0];
this._prevent=true;
E.css("display","none").css("width",this.popup.element.css("width")).css("display","inline-block");
if(F){E.focus();
l.caret(E[0],D);
}this._prevent=G;
...
},text:function(F){var G=this;
var E;
var D=G.options.ignoreCase;
F=F===null?"":F;
if(F!==C){if(typeof F!=="string"){G._textAccessor(F);
return;
}E=D?F.toLowerCase():F;
G._select(function(H){H=G._text(H);
if(D){H=(H+"").toLowerCase();
}return H===E;
}).done(function(){G._textAccessor(G.dataItem()||F);
});
}else{return G._textAccessor();
...
}E.value(G).done(function(){F._old=F._accessor();
...
this._prevent=false;
},_focusoutHandler:function(){var J=this;
var F=J._state===x;
var H=window.self!==window.top;
var G=J._focus();
var D=J._getElementDataItem(G);
var I;
if(!J._prevent){clearTimeout(J._typingTimeout);
var E=function(){if(z.mobileOS.ios&&H){J._change();
}else{J._blur();
}J._inputWrapper.removeClass(j);
J._prevent=true;
J._open=false;
J.element.blur();
};
I=!F&&G&&J._value(D)!==J.value();
if(I&&!J.trigger("select",{dataItem:D,item:G})){J._select(G,!J.dataSource.view().length).done(E);
}else{E();
...
return;
}K._select(K._focus(),true).done(function(){if(!I){K._blur();
}});
...
K._select(p(I+F,E)).done(function(){var L=function(){if(!K.popup.visible()){K._change();
}};
if(K.trigger("select",{dataItem:K._getElementDataItem(K._focus()),item:K._focus()})){K._select(H).done(L);
}else{L();
}});
...
},_click:function(D){var F=this;
var E=D.item||b(D.currentTarget);
D.preventDefault();
if(F.trigger("select",{dataItem:F._getElementDataItem(E),item:E})){F.close();
return;
}F._userTriggered=true;
F._select(E).done(function(){F._focusElement(F.wrapper);
F._blur();
});
...
}},_searchByWord:function(F){if(!F){return;
}var E=this;
...
D=F._get(D);
return F.listView.select(D).done(function(){if(!E&&F._state===x){F._state=w;
...
}if(this._isFilterEnabled()){D='<span class="k-icon k-i-zoom"></span>';
...
E=G.find(D);
if(!E[0]){G.append('<span unselectable="on" class="k-dropdown-wrap k-state-default"><span unselectable="on" class="k-input">&nbsp;</span><span unselectable="on" class="k-select" aria-label="select"><span class="k-icon k-i-arrow-60-down"></span></span></span>').append(F.element);
E=G.find(D);
}F.span=E;
